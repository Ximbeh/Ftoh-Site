...
export function CarouselCustomNavigation({championshipColorHex, newsItems}) {
    // Filtrar notícias com a tag "Highlights", pegar as últimas 6 e inverter a ordem
    const filteredNews = newsItems
        .filter(news => news.tags.includes('Highlights'))
        .slice(-6) // Pega as últimas 6
                    ...
                    >
                        {filteredNews.map((news, index) => (
                            <div className="relative" key={index}>
                                <img
                                    src={news.image || "https://media.formula1.com/image/upload/t_16by9North/f_auto/q_auto/v1719156582/fom-website/2024/Spain/GettyImages-2158857659.jpg.transform/4col/image.jpg"}
                                    alt={`image ${index + 1}`}
                        ...
                        {filteredNews.slice(0, 2).map((news, index) => (
                            <div key={index}>
                                <div className="imagem overflow-hidden inline-block rounded-sm w-full hidden md:flex">
                                    <img className="w-full h-full duration-500 transition-transform mb-2" src={news.image || "https://media.formula1.com/image/upload/t_16by9North/f_auto/q_auto/v1719156582/fom-website/2024/Spain/GettyImages-2158857659.jpg.transform/4col/image.jpg"} />
                                </div>
                                <div className="mb-4 duration-300 text pt-1 pb-3 px-3 border-r border-b border-l-0 border-t-0 border-solid border-gray-300 rounded-br-3xl min-h-4 hover:cursor-pointer">
                                    <p className="text-xs font-formula-bold mb-1 uppercase" style={{ color: championshipColorHex }}>Highlights</p>
