...
const Calendar = () => {
    ...
    const [temporarySeason, setTemporarySeason] = useState(null); // Estado local para a temporada temporária
    ...
    useEffect(() => {
        if (dataSeason) {
            // Adicione um log para verificar os dados carregados
            // console.log('Data Seasons:', dataSeason);
        }
    }, [dataSeason]);

    if (loadingChampionships || loadingSeasons) return <p>Loading...</p>;
    ...
    const actualSeason = selectedSeason.find(season => season.seasonId === championship.seasons.find(season => season.date === "2024")?.seasonId);
    ...
    return (
        <div>
            <Header championshipColorHex={championshipColorHex} ></Header>
            <main>
                <div className="max-w-xl mx-auto md:max-w-3xl lg:max-w-5xl xl:max-w-7xl">
                    <h1 className="mx-4 border-t-8 border-r-8 border-grayTotal rounded-tr-2xl mt-10 mb-10 pt-4 text-4xl md:text-6xl font-formula-bold">Calendário {championship.championshipName} {displayedSeason ? displayedSeason.date : 'N/A'} </h1>
                </div>
